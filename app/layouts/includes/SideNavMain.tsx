...
type ZegoUIKitPrebuilt = {
    new (): any; // Adjust the return type as per your Zego UI Kit documentation
};
...
export default function SideNavMain() {
    ...
    useEffect(() => {
        async function initializeZegoUIKit() {
            if (typeof window !== 'undefined') {
                const { ZegoUIKitPrebuilt } = await import('@zegocloud/zego-uikit-prebuilt');
                const zegoInstance = new ZegoUIKitPrebuilt();
                // Use zegoInstance here for initialization or save it to state if needed
            }
        }
        initializeZegoUIKit();
    }, []);
    ...
    const handleStartLiveStreaming = () => {
        // Handle starting live streaming
    };
